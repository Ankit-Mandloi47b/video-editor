import initJson from './example.json'
import initVidJson from './exampleVideo.json'
import initTextJson from './exampleText.json'
...
export const createJson = (video, textList) => {
    ...
    const TextList = textList.map( Text => {
      ...
      const {x:left, y:top, text, textDecoration, align: textAlign, background:highlightColor, fill:color, fontFamily, fontSize, fontStyle, height, width, dataId } = Text
      InitTextJson._uid = dataId

      // Text position original
// InitTextJson.style.real.top = top
      // InitTextJson.style.real.left = left
      InitTextJson.style.real.top = ((originalHeight/screenHeight)*top)+top >> 0
      InitTextJson.style.real.left = ((originalWidth/screenWidth)*left)+left >> 0
